...
def insights(request:Request,user:User=Depends(require_user),db:Session=Depends(get_db)):
 ...
 current=db.scalar(select(Sprint).where(Sprint.project_id==project.id,Sprint.status=="Active")); history=sprint_history(db,project.id); health=sprint_health(db,current) if current else None
 return templates.TemplateResponse(request,"insights.html",{"page_title":"Aria Insights","show_nav":True,"user":user,"csrf_token":csrf_token(request),"history":history,"health":health})
