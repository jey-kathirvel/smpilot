...
def chat_page(request:Request,user:User=Depends(require_user),db:Session=Depends(get_db)):
    ...
    template = "aria_widget.html" if request.query_params.get("embed") == "1" else "aria_chat.html"
    return templates.TemplateResponse(request,template,{"page_title":"Ask Aria","show_nav":True,"user":user,"csrf_token":csrf_token(request),"messages":messages})
@router.post("/aria",include_in_schema=False)
def ask(request:Request,question:str=Form(),csrf:str=Form(),embed:str=Form(default="0"),user:User=Depends(require_user),db:Session=Depends(get_db)):
